...
class TravisService : Service(), TextToSpeech.OnInitListener, RecognitionListener {
    ...
    private fun handleHypothesis(hypothesis: String?) {
        ...
        serviceScope.launch {
            // 1. Real device actions first (alarms, calls, weather, etc.)
            val handledTask = try {
            ...
            if (handledTask) return@launch

            // 2. Instant offline conversational fallback (greetings, jokes,
            // time/date, small talk). This is pattern-matched, so it's
            // effectively free - no network round trip, no waiting.
            // Brain.respond() always returns *something* (it has a catch-all
            // "I heard: ..." branch), so we treat any non-catch-all reply as
            // a match and skip Groq. We detect the catch-all by checking if
            // it starts with "I heard:" - anything else is a real match.
            val brainReply = try {
                Brain.respond(heard)
            } catch (e: Exception) {
                TravisLogger.log(this@TravisService, TAG, "Brain crashed: ${e.message}")
                null
            }

            if (brainReply != null && !brainReply.startsWith("I heard:")) {
                withContext(Dispatchers.Main) {
                    tts.speak(brainReply, TextToSpeech.QUEUE_FLUSH, null, "travis_brain_reply")
                }
                return@launch
            }

            // 3. Fall through to Groq for anything genuinely open-ended.
            try {
